...
def makeButtonSetup(colorNamesList):
    buttonSetup = []
    (x, y) = (310, 390)
    yChange = 40
    for color in colorNamesList:
        buttonSetup.append((x, y, color))
        y = y - yChange
    return buttonSetup
...
def main():
    ...
    colorList = makeButtonSetup(['red', 'yellow', 'blue', 'green', 'purple',
                                 'maroon', 'brown', 'violet', 'white'])
    for (x, y, color) in colorList:
